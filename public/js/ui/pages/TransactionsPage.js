...
class TransactionsPage {
  ...
  constructor( element ) {
    if (!element) throw new Error('Element doesnt found!');
  ...
  //Внутри обработчика пользуйтесь методами TransactionsPage.removeTransaction и
  ...
  //Удаляет счёт. 
  //Необходимо показать диаголовое окно (с помощью confirm()) 
  ...
  //либо обновляйте только виджет со счетами для обновления приложения
  removeAccount() {
  ...
  //TransactionsPage.renderTitle. Получает список Transaction.list и полученные 
  //данные передаёт в TransactionsPage.renderTransactions()
  render(options){
    if (!options) return;
